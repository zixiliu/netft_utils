...
#define COMMAND 0 /* Command code 2 starts streaming */
#define NUM_SAMPLES 3 /* Will send 1 sample before stopping */
	...
	*(uint32*)&request[4] = htonl(NUM_SAMPLES); /* see section 9.1 in Net F/T user manual. */
	...
	if (NUM_SAMPLES == 0) {
		while(1){
			...
			printf( "seq num of this packet: 0x%08x\n", resp.rdt_sequence );
			printf( "internal seq num : 0x%08x\n", resp.ft_sequence );
